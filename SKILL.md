...
2. 使用当前 Hermes 会话作为通知目标。底层会读取 `HERMES_SESSION_PLATFORM` 与 `HERMES_SESSION_CHAT_ID`；Hook 也会保存待确认目标。
3. 若只检测到一个有效 Himalaya 配置，自动复用；不要询问已经可靠检测到的账户或路径。
4. 只逐项询问缺失的用户级信息：
   - 要监控哪些邮箱；
   - 仅行动项还是更广泛通知；
   - 附件自动下载/微信转发偏好和大小上限；
   - 时区与提醒提前量；
   - 模型偏好（无偏好时保留默认）。
5. 新账户必须使用外部凭据命令，例如 `pass`、`secret-tool`、`security`、`op`、`bw`、`gopass` 或受保护的环境变量。绝不询问、接收或复述邮箱密码、应用专用密码、令牌和密钥；禁止用带明文的 `echo`/`printf`。
6. 由 Agent 在后台构造设置数据，并依次调用：
...
7. 普通“安装”默认保持禁用；明确的“安装并启用”或“启用邮件监控”才构成启用授权。
8. 应用后运行 `doctor` 与 `run-once`。只报告脱敏结果，不暴露完整邮箱、聊天 ID、凭据命令或内部路径。
9. 只有只读验证和试运行通过后才运行 `enable`。恢复或升级时跳过已经验证的步骤，不重复要求登录。
