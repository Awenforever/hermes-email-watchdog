...
- `deepseek-flash` 先理解邮件事实与用户真正需要，再执行一次独立的发布前审阅：决定是否值得打扰、怎样组织最终微信 Markdown、哪些链接和附件真正有用，以及相对期限在当前时间是否仍有效。任一步失败都会回退到 `qwen3.8-chat`，而不是靠不断增加邮件类型专用规则。
- 程序只负责来源校验、安全边界、幂等投递和附件/日程等真实执行结果；不会用关键词分类器替代模型对新邮件的判断，也不再套用统一的“要点 + 原文摘录”模板。
...
- 默认复用 Hermes 中名为 `USTC` 的 OpenAI 兼容提供方，主模型为
  `deepseek-flash`，失败时回退到 `qwen3.8-chat`；自定义配置会在升级时保留。
...
- 微信通知按内容灵活使用标题、行内代码、列表、链接、引用和语义化小节；发件人、主题、编号、金额和时间等精确信息用行内代码呈现。转发分隔符、邮件头、问候、签名、退订文字和“无附件”等机械描述不会成为要点；原文只在精确措辞确有价值时保留为一段连贯依据。
- 附件、日程或持久化等局部阶段失败时会独立记录并降级，不会让已经验证过的模型理解退回旧式正文切片格式。
...
可选设置包括：主模型与 fallback、仅行动项或全部邮件通知、是否自动下载附件、是否转发到微信、附件大小上限、时区和提醒提前量。推荐保持默认值：`deepseek-flash` → `qwen3.8-chat`、仅行动项通知、自动下载并转发安全附件、单个附件最多 25 MiB、提前 24 小时和 1 小时提醒。
