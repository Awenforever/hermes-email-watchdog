...
配置、seen 索引、缓存、学习数据库、状态和 outbox 位于当前 Hermes profile 的 `plugin-data/hermes-email-watchdog/`。安装和升级不会覆盖邮箱数据、认证、目标或个性化配置；从旧版升级时，仅在新位置没有对应文件的情况下迁移旧数据。0.3.0 只会识别并迁移 0.2.x 的完整默认策略签名，迁移前自动备份，插件回滚时同步恢复；任一策略字段经过自定义都会原样保留。
