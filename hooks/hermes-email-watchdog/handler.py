    ...
    for entry in _outbox_pending_entries(limit=limit, due_only=True):
        ...
        try:
            result = await _send_weixin(
                entry.get("text", ""), delivery_id=entry.get("delivery_id") or entry.get("id")
            )
...
def _call_watchdog() -> str:
    ...
    getter = getattr(email_watch, "get_last_output_metadata", None)
    try:
    ...
    model_name = str(metadata.get("model_name") or "hermes").strip() or "hermes"
...
async def _send_weixin(
    ...
    if not isinstance(attribution, dict):
        ...
        try:
            with _state_file_lock(OUTBOX_FILE):
                data = _outbox_load()
                stored = (data.get("entries") or {}).get(delivery_id)
                if isinstance(stored, dict) and isinstance(stored.get("metadata"), dict):
                    attribution = dict(stored.get("metadata") or {})
        except Exception:
            attribution = {}
    model_generated = bool(attribution.get("model_generated"))
    model_name = str(attribution.get("model_name") or "hermes").strip() or "hermes"
