...
def _has_non_negated_match(pattern, text):
    """Return true when a regex match is outside a negated clause."""
    boundary = re.compile(
        r"(?:[\n\r.!?。！？;；]+|\bbut\b|\bhowever\b|\bnevertheless\b|"
        r"\byet\b|但|但是|不过|然而)",
        re.I,
    )
    negation = re.compile(
        r"(?:\bno\b|\bnot\b|\bnone\b|\bwithout\b|\bneither\b|\bnor\b|"
        r"没有|并无|不含|不包含|不涉及|不存在|未包含|未涉及|未要求|"
        r"无需|无须|无)",
        re.I,
    )
    post_negation = re.compile(
        r"^\s*(?:(?:is|are|was|were|be|being|been|has|have|had)\s+)?"
        r"(?:not\b|absent\b|unavailable\b)|"
        r"^\s*(?:不存在|未包含|不需要|无需|无须)",
        re.I,
    )
    for match in re.finditer(pattern, text or "", re.I | re.S):
        left = (text or "")[max(0, match.start() - 240):match.start()]
        parts = boundary.split(left)
        clause_left = parts[-1] if parts else left
        right = (text or "")[match.end():match.end() + 100]
...
def classify(email_data):
    ...
    for kw in invoice_kw:
        if _has_non_negated_match(re.escape(kw), full_text):
    ...
    for kw in pay_kw:
        if _has_non_negated_match(kw, full_text):
...
def _delivery_owns_learning_record(delivery):
    """The production delivery route persists the canonical semantic decision.

    Avoid overwriting that row later with the legacy placeholder analysis that
    merely existed as a formatter fallback input.
    """
    if not isinstance(delivery, dict):
        return False
    return str(delivery.get("production_route") or "") in {
        "adaptive_v1e", "adaptive_v1f", "adaptive_v1g", "intelligent_v2", "legacy_fallback",
    }
...
def check_account(acct, pushed_count=None):
    ...
    for env in envelopes:
        ...
        if HAS_V3:
            ...
            # The active production route records its canonical semantic decision
            # inside email_delivery. Do not overwrite it here with the legacy
            # placeholder analysis retained only for fallback formatting.
            if HAS_LEARNING and not _delivery_owns_learning_record(delivery):
                try:
