...
def record_decision(email: dict, rule_result: dict, analysis: dict, delivery: Optional[dict] = None, account: Optional[dict] = None) -> dict:
    try:
        ...
        semantic_meta = delivery.get("semantic") if isinstance(delivery.get("semantic"), dict) else {}
        semantic_model = str(semantic_meta.get("model") or "").strip().lower()
        used_llm = int(bool(
            analysis.get("llm_raw")
            or analysis.get("llm_model")
            or analysis.get("llm_used")
            or semantic_meta.get("llm_called")
            or (semantic_model and semantic_model != "hermes")
        ))
        if str(analysis.get("llm_notes") or "").lower().startswith("rule bypass"):
            used_llm = 0
        if "llm disabled" in _json_safe(analysis).lower():
            used_llm = 0
        if str(delivery.get("route_lane") or "").lower() == "fast":
            used_llm = 0
