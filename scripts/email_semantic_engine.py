...
PROMPT_VERSION = "semantic_v2_tolerant_grounded_core_v1x_20260923"
...
def _raw_semantic_fields(parsed: Any) -> Dict[str, str]:
    """Extract privacy-bounded model fields for post-incident tracing.

    The raw model response and source email are never persisted. Only concise
    semantic verdicts and the model-authored summary are retained, each with a
    strict length cap, so grounding changes remain auditable.
    """
    if not isinstance(parsed, Mapping):
        return {
            "raw_category": "", "raw_importance": "",
            "raw_risk_level": "", "raw_summary": "",
        }
    ...
    summary = str(parsed.get("summary") or "").replace("\x00", "").strip()
    if not summary:
        points = parsed.get("key_points")
        if isinstance(points, list):
            summary = "\n".join(str(item).strip() for item in points[:5] if str(item).strip())
    return {
        "raw_category": str(parsed.get("category") or "")[:120],
        "raw_importance": str(parsed.get("importance") or "")[:40],
        "raw_risk_level": risk_level[:40],
        "raw_summary": summary[:600],
    }


def _trace_signals(facts: Mapping[str, Any]) -> Dict[str, bool]:
    hints = facts.get("semantic_hints") if isinstance(facts, Mapping) else {}
    ...
    keys = (
        ...
        "school_institution_phrase", "risk_phrase",
    )
    return {key: bool(hints.get(key)) for key in keys}
...
def _semantic_hints(email: Mapping[str, Any], subject: str, body: str, features: Mapping[str, Any]) -> Dict[str, bool]:
    ...
    def has_non_negated(pattern: str, text: str = combined) -> bool:
        """Return true only when a matching fact is not inside a negated clause.

        This prevents phrases such as "there is no deadline, invoice or payment
        request" from becoming positive deadline/receipt evidence. Sentence and
        contrast boundaries reset the negation scope so mixed statements remain
        detectable.
        """
        boundary = re.compile(
            r"(?:(?:\r?\n){2,}|[.!?。！？;；]+|\bbut\b|\bhowever\b|\bnevertheless\b|"
            r"\byet\b|但|但是|不过|然而)",
            re.I,
        )
        negation = re.compile(
            r"(?:\bno\b|\bnot\b|\bnone\b|\bwithout\b|\bneither\b|"
            r"\bnor\b|\babsent\b|没有|并无|不含|不包含|不涉及|不存在|"
            r"未包含|未涉及|未要求|未规定|无需|无须|无)",
            re.I,
        )
        post_negation = re.compile(
            r"^\s*(?:(?:is|are|was|were|be|being|been|has|have|had)\s+)?"
            r"(?:not\b|absent\b|unavailable\b|not\s+required\b|not\s+imposed\b)|"
            r"^\s*(?:不存在|未规定|未要求|不需要|无需|无须)",
            re.I,
        )
        for match in re.finditer(pattern, text, re.I | re.S):
            left = text[max(0, match.start() - 240):match.start()]
            parts = boundary.split(left)
            clause_left = parts[-1] if parts else left
            right = text[match.end():min(len(text), match.end() + 100)]
            if negation.search(clause_left):
                continue
            if post_negation.search(right):
                continue
            return True
        return False
    ...
    spam_subject_pattern = (
        r"^\s*\[(?:spam|junk|bulk|垃圾邮件|广告邮件)\]"
    )
    publication_issue_pattern = (
        r"(?:read|view|explore|discover|browse)\s+(?:the\s+)?(?:latest|new|current)\s+issue\b|"
        r"(?:latest|new|current)\s+issue\s+of\b|"
        r"阅读|查看|浏览|探索.{0,8}(?:最新|本期|新一期).{0,8}(?:期刊|杂志|内容)"
    )
    return {
        "no_action_phrase": has(
            r"no (?:response|reply)(?: or (?:immediate )?action)? (?:is )?required|"
            r"no (?:immediate )?action (?:is )?required|"
            r"无需操作|无需回复|不需要操作|仅供参考"
        ),
        ...
        "no_receipt_phrase": has(
            r"(?:\bno\b|\bwithout\b).{0,120}(?:receipt|invoice|payment request|bill)|"
            r"(?:receipt|invoice|payment request|bill).{0,40}(?:is|are|was|were)?\s*not\b|"
            r"(?:没有|并无|不含|不包含|不存在|未包含).{0,40}(?:收据|发票|账单|付款请求)"
        ),
        "receipt_phrase": has_non_negated(
            r"收据|发票|receipt|invoice|payment successful|付款成功|支付成功|账单"
        ),
        ...
        "spam_subject_phrase": has(spam_subject_pattern, subject),
        "publication_issue_phrase": has(publication_issue_pattern),
        "publication_issue_subject_phrase": has(publication_issue_pattern, subject),
        ...
        "newsletter_marketing_phrase": bool(
            has(
    ...
    try:
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS semantic_observations (
              ...
              normalization_repairs_json TEXT,
              trace_signals_json TEXT,
    ...
    try:
        ...
        row = conn.execute(
            ...
            SELECT decision_json, model, prompt_version, schema_version, latency_ms,
                   fallback_used, timeout, error_code, llm_called, decision_source,
                   raw_category, raw_importance, raw_risk_level, raw_summary,
                   validated_category, fallback_reason, normalization_repairs_json, trace_signals_json,
                   production_route
        ...
        return {
            ...
            "raw_importance": row["raw_importance"] or "",
            "raw_risk_level": row["raw_risk_level"] or "",
            "raw_summary": row["raw_summary"] or "",
            "validated_category": row["validated_category"] or "",
            "fallback_reason": row["fallback_reason"] or "",
            "normalization_repairs": json.loads(row["normalization_repairs_json"] or "[]"),
            "trace_signals": json.loads(row["trace_signals_json"] or "{}"),
            "production_route": row["production_route"] or "",
        }
    except Exception:
        return None
    finally:
...
def _persist_observation(
    *,
    ...
    raw_summary: str = "",
    fallback_reason: str = "",
    normalization_repairs: list[str] | None = None,
    trace_signals: Mapping[str, Any] | None = None,
    ...
    try:
        ...
        conn.execute(
            ...
            INSERT INTO semantic_observations (
              ...
              error_code, llm_called, decision_source, raw_category,
              raw_importance, raw_risk_level, raw_summary, validated_category,
              fallback_reason, normalization_repairs_json, trace_signals_json,
              production_route, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(message_key) DO UPDATE SET
              ...
              normalization_repairs_json=excluded.normalization_repairs_json,
              trace_signals_json=excluded.trace_signals_json,
              production_route=excluded.production_route,
            ...
            (
                ...
                error_code[:200],
                int(bool(llm_called)),
                str(decision_source or "")[:80],
                str(raw_category or "")[:120],
                str(raw_importance or "")[:40],
                str(raw_risk_level or "")[:40],
                str(raw_summary or "")[:600],
                str(classification.get("category") or "unknown_needs_llm")[:120],
                str(fallback_reason or "")[:500],
                _json(list(normalization_repairs or [])[:40]),
                _json(dict(trace_signals or {})),
                str(production_route or "")[:80],
                now,
                now,
            ),
        )
...
def analyze_email(
    ...
    *,
    ...
    return {
        ...
        "decision_source": "conservative_fallback" if fallback_used else "llm_validated",
        "raw_category": raw_category,
        "raw_importance": raw_importance,
        "raw_risk_level": raw_risk_level,
        "raw_summary": raw_summary,
        "validated_category": str((validated.get("classification") or {}).get("category") or ""),
        "fallback_reason": fallback_reason,
        "trace_signals": _trace_signals(facts),
...
def persist_production_observation(
    email: Mapping[str, Any],
    ...
    *,
    ...
    """Persist a privacy-bounded trace for the actual production semantic route.

    This stores verdict/provenance fields and hashes, never the raw email body or
    full model response. Failure is returned to the caller and must not change
    notification delivery.
    """
    try:
        result = dict(semantic_result or {})
        decision = result.get("decision")
        if not isinstance(decision, Mapping):
            return {"ok": False, "error": "semantic decision missing"}
        features = email_feature_extractor.extract_features(dict(email or {}))
        facts = result.get("facts")
        if not isinstance(facts, Mapping):
            facts = _facts(email or {}, features)
        message_key = str(result.get("message_key") or features.get("message_key") or "")
        if not message_key:
            return {"ok": False, "error": "message_key missing"}
        ...
        return _persist_observation(
            ...
            facts=facts,
            decision=decision,
            model=str(result.get("model") or ("hermes" if not result.get("llm_called") else "")),
            ...
            decision_source=str(result.get("decision_source") or ("fast_lane" if result.get("fast_lane") else "")),
            raw_category=str(result.get("raw_category") or ""),
            raw_importance=str(result.get("raw_importance") or ""),
            raw_risk_level=str(result.get("raw_risk_level") or ""),
            raw_summary=str(result.get("raw_summary") or ""),
            fallback_reason=str(result.get("fallback_reason") or result.get("production_route_error") or ""),
            normalization_repairs=list(result.get("normalization_repairs") or []),
            trace_signals=result.get("trace_signals") if isinstance(result.get("trace_signals"), Mapping) else _trace_signals(facts),
            production_route=str(production_route or result.get("production_route") or ""),
        )
    except Exception as exc:
    ...
    try:
        ...
        if cached is not None:
            persist = _persist_observation(
                ...
                llm_called=cached.get("llm_called", False),
                decision_source="cache",
                raw_category=cached.get("raw_category", ""),
                raw_importance=cached.get("raw_importance", ""),
                raw_risk_level=cached.get("raw_risk_level", ""),
                fallback_reason=cached.get("fallback_reason", ""),
                normalization_repairs=cached.get("normalization_repairs") or [],
                trace_signals=cached.get("trace_signals") or {},
                production_route=cached.get("production_route", "shadow_cache"),
            )
            return {
                "ok": bool(persist.get("ok")),
                "cache_hit": True,
                "llm_called": False,
                "origin_llm_called": cached.get("llm_called", False),
                "decision_source": "cache",
                "raw_category": cached.get("raw_category", ""),
                "validated_category": cached.get("validated_category", ""),
                "fallback_reason": cached.get("fallback_reason", ""),
                "normalization_repairs": cached.get("normalization_repairs") or [],
                "trace_signals": cached.get("trace_signals") or {},
                "message_key": message_key,
        ...
        return {
            ...
            "raw_importance": result.get("raw_importance"),
            "raw_risk_level": result.get("raw_risk_level"),
            "validated_category": result.get("validated_category"),
            "fallback_reason": result.get("fallback_reason"),
            "trace_signals": result.get("trace_signals") or {},
